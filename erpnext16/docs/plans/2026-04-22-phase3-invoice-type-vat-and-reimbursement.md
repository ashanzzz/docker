...
### Task 3: 前端补发票类型联动
**Objective:** 采购发票前端切换发票类型后，税桥接与发票号必填状态同步更新。
...
**Checks:**
- 发票类型切换会重新桥接税费
- 专票/普票时 `bill_no` 必填并提示填写真实票号
- `无发票` 时 `bill_no` 留空，不再写 `0`
- 税费总额刷新正常
### 补充设计：`无发票` 的 `bill_no` 应该怎么设计

当前正式规则：
- `custom_invoice_type = 无发票` 时，`bill_no` 留空
- `custom_invoice_type = 专用发票 / 普通发票` 时，`bill_no` 必须填写真实票号
- 旧数据中的 `bill_no = 0` 只作为兼容迁移信号，不再作为新录入写法

这样做的原因：
- `bill_no` 本质是外部票据编号，不应该承载“无票状态”语义
- 用空值 + 发票类型字段，能避免打印、接口、对账把 `0` 误认成真实票号
- 前端和服务端都能明确做“有票时必填、无票时留空”的条件校验
...
### 一句话结论
原先的 `无发票 -> bill_no = 0` 现在应视为 **旧版过渡做法**；当前正式设计已改成：
- `bill_no` 只存真实票号
- `无发票` 时 `bill_no` 留空
