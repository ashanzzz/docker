...
- `custom_invoice_type`（必填）
- 录入顺序建议：先选发票类型，再填发票号

### 发票类型与发票号规则

| 发票类型 | 发票号 `bill_no` | 发票日期 `bill_date` | VAT - 祺富 | 说明 |
| --- | --- | --- | --- | --- |
| 专用发票 | 必填，填真实票号 | 建议必填 | 进入 | 可抵扣 |
| 普通发票 | 必填，填真实票号 | 建议必填 | 不进入 | 不可抵扣 |
| 无发票 | 留空 | 可空 | 不进入 | 发票号不再使用 `0` 占位 |

### 实际规则
- 专用发票：税额桥接到可抵扣 VAT 科目，当前为 `VAT - 祺富`
- 普通发票：不进入可抵扣 VAT，走非抵扣税费逻辑
- 无发票：发票号留空，不再写 `0`

### 说明
- 以前的 `bill_no = 0` 只作为旧数据兼容，不是新单据的默认写法
- 录入时建议先选发票类型，再看发票号是否需要填写
...
### 业务含义
- **专用发票**：可抵扣税额进入 VAT，`bill_no` 必填真实票号
- **普通发票**：税额不进入可抵扣 VAT，`bill_no` 必填真实票号
- **无发票**：不进 VAT，且 `bill_no` 留空
