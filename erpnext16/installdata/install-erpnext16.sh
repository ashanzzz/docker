...
# 会安装 Python(>=3.14)，mariadb，redis 以及 ERPNext 的其他系统需求。
...
DEBIAN_FRONTEND=noninteractive apt install -y \
    ...
    libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev libffi-dev liblzma-dev libncursesw5-dev libgdbm-dev libgdbm-compat-dev uuid-dev tk-dev

echo "===================安装 Python 3.14（ERPNext v16 需要 >=3.14）==================="
PYTHON_VERSION="${PYTHON_VERSION:-3.14.0}"

if ! python3 -V 2>/dev/null | grep -q "3.14"; then
  ...
  /usr/local/bin/python3.14 -m ensurepip
...
if type python3 >/dev/null 2>&1; then
    result=$(python3 -V | grep "3.14" || true)
    if [[ "${result}" == "" ]]
    then
        echo '==========已安装python3，但不是推荐的3.14版本。=========='
        warnArr[${#warnArr[@]}]="Python不是推荐的3.14版本。"
    else
        echo '==========已安装python3.14=========='
    fi
...
for ((i=0; i<5; i++)); do
    rm -rf ~/${installDir}
    set +e
    bench init ${frappeBranch} --python /usr/local/bin/python3 --ignore-exist ${installDir} ${frappePath}
    err=\$?
    set -e
    if [[ \${err} == 0 ]]; then
        echo "执行返回正确\${i}"
